...
const { saveSessionToDb, loadSessionFromDb } = require("./db");
	...
	io.on("connection", (socket) => {
		...
		const broadcastToSession = (
			...
			if (session) {
				session.users.forEach((userInfo, userId) => {
					if (
						userInfo.socketId &&
						userInfo.socketId !== excludeSocketId
					) {
		...
		socket.on("join_session", async (data) => {
			...
			try {
				...
				if (!sessionId || !userId || !username) {
					socket.emit("error", {
						message:
							"Missing required fields: sessionId, userId, username",
					});
					return;
				}
				...
				if (currentSessionId) {
					socket.emit("error", {
						message:
							"Already in a session. Leave current session first.",
					});
					return;
				}
				...
				console.log(
					`User ${username} (${userId}) joined session ${sessionId}`
				);
			} catch (error) {
		...
		socket.on("sync_playback", async (data) => {
			try {
				...
				if (session.leaderId !== currentUserId) {
					socket.emit("error", {
						message: "Only the leader can control playback",
					});
					return;
				}
		...
		socket.on("request_leadership", async () => {
			try {
				...
				if (!currentLeader || !currentLeader.isConnected) {
					...
					socket.emit("error", {
						message: "Current leader is still active",
					});
				}
			} catch (error) {
				console.error("Error in request_leadership:", error);
				socket.emit("error", {
					message: "Failed to request leadership",
				});
			}
		});
		...
		socket.on("report_desync", async (data) => {
			try {
				...
				console.log(
					`Desync reported by user ${currentUserId} in session ${currentSessionId}`
				);
			} catch (error) {
				console.error("Error in report_desync:", error);
				socket.emit("error", {
					message: "Failed to handle desync report",
				});
			}
		});
		...
		socket.on("load_media", async (data) => {
			try {
				...
				if (session.leaderId !== currentUserId) {
					socket.emit("error", {
						message: "Only the leader can load media",
					});
					return;
				}
				...
				if (currentSessionId && currentUserId) {
					...
					if (session) {
						...
						if (connectedUsers.length === 0) {
							console.log(
								`Session ${currentSessionId} has no connected users`
							);
	...
	setInterval(
		async () => {
			...
			for (const [sessionId, session] of activeSessions.entries()) {
				...
				const isInactive =
					now - session.lastActivity > CLEANUP_THRESHOLD;
